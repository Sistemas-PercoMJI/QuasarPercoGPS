...
exports.enviarTicketSoporte = onRequest(
  {
    cors: true,
    secrets: ['NOREPLY_EMAIL_PASSWORD'],
    memory: '512MiB',
  ...
  async (req, res) => {
    try {
      logger.log('🎫 Ticket de soporte recibido')

      if (req.method !== 'POST') {
        return res.status(405).json({ success: false, error: 'Método no permitido. Use POST.' })
      }

      const { nombre, empresa, correoCuenta, correoSeguimiento, descripcion, imagenes } = req.body

      if (!nombre || !empresa || !correoCuenta || !correoSeguimiento || !descripcion) {
        logger.warn('❌ Faltan campos requeridos')
        return res.status(400).json({ success: false, error: 'Faltan campos requeridos' })
      }
      ...
      // Construir el bloque de imágenes adjuntas si las hay
        ...
        imagenesArray.length > 0
          ...
          <tr>
            <td style="padding: 20px 36px 0;">
              <span style="font-size:11px; color:#999999; text-transform:uppercase; letter-spacing:0.5px;">Imágenes adjuntas (${imagenesArray.length})</span>
              <div style="margin-top:12px; display:flex; gap:12px; flex-wrap:wrap;">
                ${imagenesArray
                  .map(
                    (img) => `
                  <div style="margin-bottom:12px;">
                    <p style="margin:0 0 4px; font-size:11px; color:#aaaaaa;">${img.nombre}</p>
                    <img
                      src="data:${img.tipo};base64,${img.base64}"
                      alt="${img.nombre}"
                      style="max-width:440px; width:100%; border-radius:6px; border:1px solid #e0e0e0;"
                    />
                  </div>
                `,
                  )
      ...
      const mailOptions = {
        from: `"PercoGPS" <${NOREPLY_EMAIL}>`,
        to: SISTEMAS_EMAIL,
        subject: `Ticket de soporte — ${nombre} (${empresa})`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
                  ...
                  <table width="560" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
                    ...
                    <tr>
                      <td style="padding: 24px 36px 0;">
                        <p style="margin:0 0 16px; font-size:13px; color:#555555;">Se ha recibido un nuevo ticket de soporte técnico con los siguientes datos:</p>
                        <table width="100%" cellpadding="0" cellspacing="0">
                          <tr>
                            <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f0;">
                              <span style="font-size:11px; color:#999999; text-transform:uppercase; letter-spacing:0.5px;">Nombre</span><br>
                              <span style="font-size:14px; color:#1a1a1a; font-weight:600;">${nombre}</span>
                            </td>
                          </tr>
                          <tr>
                          ...
                          <tr>
                            <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f0;">
                              <span style="font-size:11px; color:#999999; text-transform:uppercase; letter-spacing:0.5px;">Correo de cuenta</span><br>
                              <span style="font-size:14px; color:#1a1a1a; font-weight:600;">${correoCuenta}</span>
                            </td>
                          </tr>
                          <tr>
                            <td style="padding: 10px 0; border-bottom: 1px solid #f0f0f0;">
                              <span style="font-size:11px; color:#999999; text-transform:uppercase; letter-spacing:0.5px;">Correo de seguimiento</span><br>
                              <span style="font-size:14px; color:#1a1a1a; font-weight:600;">${correoSeguimiento}</span>
                            </td>
                          </tr>
                          <tr>
                            <td style="padding: 10px 0;">
                              <span style="font-size:11px; color:#999999; text-transform:uppercase; letter-spacing:0.5px;">Descripción del problema</span><br>
                              <span style="font-size:14px; color:#1a1a1a; white-space:pre-wrap;">${descripcion}</span>
                            </td>
                          </tr>
                        </table>
                      </td>
                    ...
                    <tr>
                      <td style="padding: 24px 36px 0;"></td>
                    </tr>

                    <!-- Footer -->
                    <tr>
                      <td style="background-color:#f8f8f8; padding: 16px 36px; border-top: 1px solid #eeeeee;">
                        <p style="margin:0; font-size:11px; color:#aaaaaa; text-align:center;">Este es un correo automático generado por PercoGPS. No responder a este mensaje.</p>
                      </td>
                    </tr>
      ...
      await transporter.sendMail(mailOptions)
      logger.log(`✅ Ticket de soporte enviado — ${nombre} (${empresa})`)
