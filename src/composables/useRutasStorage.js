...
export function useRutasStorage() {
  ...
  const procesarColaReintentos = async () => {
    ...
    while (colaReintentos.value.length > 0) {
      ...
      try {
        ...
        if (item.intentos < 3) {
          console.log(
            `⏳ Agregando de nuevo a la cola: ${item.unidadId}-${item.fecha} (intento ${item.intentos})`,
          )
  ...
  const guardarBatchEnStorage = async (unidadId, fecha, datos, retryCount = 0) => {
    ...
    try {
      ...
      for (let i = 0; i < partes.length; i++) {
        ...
        console.log(
          `⬆️ Intentando subir${parte.esParte ? ` parte ${parte.parte}` : ''} a Storage: ${rutaArchivo}`,
        )
        ...
        console.log(
          `✅ Archivo${parte.esParte ? ` parte ${parte.parte}` : ''} subido exitosamente: ${rutaArchivo}`,
        )
      }
      ...
      if (
        ...
        console.warn(
          `⚠️ Error de conexión. Reintentando en ${delay / 1000}s (intento ${retryCount + 1}/3)...`,
        )
        ...
        console.error(
          `🛑 Máximo de reintentos alcanzado para ${unidadId}-${fecha}. Agregando a la cola de reintentos.`,
        )
  ...
  const obtenerCoordenadasDesdeStorageDividido = async (urlIndice) => {
    try {
      ...
      console.log(
        `✅ Descargadas ${todasLasCoordenadas.length} coordenadas de ${indice.partes.length} partes`,
      )
  ...
  const obtenerCoordenadasDesdeStorage = async (url) => {
    ...
    try {
      ...
      // 🔥 CAMBIO: Leer y parsear JSON de una vez
      const data = await response.json()
      console.log('📄 Tamaño:', JSON.stringify(data).length, 'bytes')

      console.log('📊 Tipo de dato:', typeof data)
      console.log('📊 Es array:', Array.isArray(data))
      console.log('📊 Tiene .coordenadas:', !!data.coordenadas)

      // 🔥 MANEJAR AMBOS FORMATOS
      let coordenadas = []

      if (Array.isArray(data)) {
        // Formato antiguo (24 nov): array directo
        console.log('📦 Formato antiguo detectado (array directo)')
        coordenadas = data
      } else if (data.coordenadas && Array.isArray(data.coordenadas)) {
        // Formato nuevo (25/26 nov): objeto con .coordenadas
        console.log('📦 Formato nuevo detectado (objeto con .coordenadas)')
        console.log(`📦 Total items: ${data.coordenadas.length}`)

        // Transformar cada coordenada del formato nuevo al esperado
        coordenadas = data.coordenadas
          .map((item, index) => {
            // Extraer la coordenada anidada
            const coord = item.nuevaCoordenada || item

            // Validar que tenga lat/lng
            if (!coord.lat || !coord.lng) {
              console.warn(`⚠️ Coordenada ${index} sin lat/lng:`, item)
              return null
            }
            ...
            return {
              ...
              // 🔥 CALCULAR ignicion por velocidad (si velocidad > 0 = motor encendido)
              ignicion: parseFloat(item.velocidad_actual || coord.velocidad || 0) > 0,
              velocidad: parseFloat(item.velocidad_actual || coord.velocidad || 0),
        ...
        console.log(`✅ ${coordenadas.length} coordenadas transformadas`)
        if (coordenadas.length > 0) {
          console.log('📦 Primera coordenada:', coordenadas[0])
          console.log('📦 Última coordenada:', coordenadas[coordenadas.length - 1])
        }
      } else {
        console.warn('⚠️ Formato desconocido:', data)
        return []
      }

      if (coordenadas.length === 0) {
        console.warn('⚠️ Array de coordenadas vacío')
        return []
      }

      console.log(`✅ ${coordenadas.length} coordenadas listas para usar`)
      return coordenadas
    } catch (err) {
      error.value = err.message
